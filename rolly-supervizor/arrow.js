...
//------------------------------------------------------------------------------------------------------------\\
  ...
  client.on("message", message => {
    ...
    return message.channel.send(("³²¹  & #0321"))
});

client.on("message", message => {
  if(message.content.toLowerCase() == ".tag") 
  return message.channel.send(("³²¹  & #032"))
});
...
client.on("message", async message => {
  ...
  client.on("userUpdate", async function(oldUser, newUser) {
    const guildID = "903899490992422994"//sunucu
    const roleID = "924702705249837056"//taglırolü
    const tag = "³²¹"//tag
    const chat = '924702759566061688'// chat
    const log2 = '924702795385434132' // log kanalı
   ...
   if (newUser.discriminator !== oldUser.discriminator) {
        if (oldUser.discriminator == "321" && newUser.discriminator !== "321") {
            member.roles.remove(roleID)
            client.channels.cache.get(log2).send(embed.setDescription(`  ${newUser} etiketinden \`321\` çıakrtarak ailemizden ayrıldı!`))
        } else if (oldUser.discriminator !== "321" && newUser.discriminator == "321") {
            member.roles.add(roleID)
            client.channels.cache.get(log2).send(embed.setDescription(`  ${newUser} etiketine \`321\` alarak ailemize katıldı`))
            client.channels.cache.get(chat).send(`Tebrikler, ${newUser} tag alarak ailemize katıldı ona sıcak bir **'Merhaba!'** diyin.(#0321)`)
        }
    }
...
client.on("guildMemberAdd", member => {
  ...
  let sunucuid = "903899490992422994"; 
  var tag = '³²¹';
  let rol = "924702705249837056";
...
const mutelogs = new Discord.WebhookClient(config.mutelogid, config.mutelogtoken)
  ...
  if(muteData) {
      member.roles.add("924702723541200908") //Muted ID
...
client.on("guildMemberAdd", member => {  
 const lorewebhook = new Discord.WebhookClient('924702745229930566', '4o5lp-jzW9qlkUZ9LZj91uFz9k8rEdy4OId7eVmAgHKTTqQTaz9SuetVqtUN5G9TZ5_1')
   ...
   const gecen = moment.duration(kurulus).format(`YY **[Yıl,]** DD **[Gün,]** HH **[Saat,]** mm **[Dakika,]** ss **[Saniye]**`) 
    ...
    moment.locale("tr");
  lorewebhook.send(`\`>\` Sunucumuza hoşgeldin ${member}! Seninle birlikte ${member.guild.memberCount} kişiye ulaştık.
    
  \`>\` Hesabın ` +gecen+ ` açılmış.
  
    \`>\` Sunucu kurallarımız <#924702752674824232> kanalında belirtilmiştir. Unutma! Sunucu içerisindeki tutumun kuralların izin verdiği sınırları aşarsa gerekli ceza-i işlemler uygulanır.
  
  \`>\` Tagımızı **³²¹** ve etiketimizi **#321** alarak bizlere destek olabilirsin.
  
\`>\` Sol taraftaki \`V.Confirmed\` kanallarından birine girip \`isim-yaş\` belirterek kayıt olabilirsin. <@&924702702938755083>`);
...
client.on('messageDelete', (message) => {
 ...
 return client.channels.cache.get("928360365853671454").send(embed)
})
//------------------------------------------------------------------------------------------------------------\\
...
client.on("guildMemberAdd", member => {
  member.roles.add('924702710102626324'); // UNREGİSTER ROLÜNÜN İDSİNİ GİRİN
});
//-----------------------GİRENE-ROL-VERME----------------------\\     
