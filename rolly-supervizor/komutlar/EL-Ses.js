...
module.exports.run = async(client, message, args) => {
    ...
    let pub = message.guild.channels.cache.filter(x => x.parentID === "924702737797615686");
    let public = 0;
    for (const [id, voiceChannel] of pub)
    public += voiceChannel.members.size;

    let priv = message.guild.channels.cache.filter(x => x.parentID === "924702739538260029");
    let privkanalları = 0;
    for (const [id, voiceChannel] of priv)
    privkanalları += voiceChannel.members.size;

    let alone = message.guild.channels.cache.filter(x => x.parentID === "924702740632981565");
